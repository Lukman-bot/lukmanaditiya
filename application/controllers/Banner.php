...
class Banner extends CI_Controller {
	...
	public function ajax_list()
   {
      ...
      foreach($list as $li){
         ...
         $row[] = 
         '<a class="btn btn-sm btn-warning text-white" href="'.base_url("index.php/banner/edit/$li->id").'" 
         title="Edit">
	...
	public function add()
	{
		...
		if($this->form_validation->run() == false){
			...
			$data['form_action'] = base_url("index.php/banner/add");
			$data['input'] 		= $input;
			$this->load->view('admin/layouts/main', $data);
		}else{
	...
	public function edit($id)
	{
		...
		if($this->form_validation->run() == false){
			...
			$data['input']			= $input;
			// var_dump($data['input']);
			$data['form_action']	= base_url('index.php/banner/edit/' . $id);
			$this->load->view('admin/layouts/main', $data);
		}else{
			$data = [
				'title' => $this->input->post('title', true),
			...
			if(!empty($_FILES['photo']['name'])){
				...
				if($upload){
					...
					redirect(base_url("index.php/banner/edit/$id"));
				}
			}
